...
/*   Updated: 2023/06/01 10:13:55 by rbarbiot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line_ml_fd(int fd)
{
	char		*target_l;
	char		*tmp;
	static char	*previous_ls[OPEN_MAX];

	if (fd < 0 || BUFFER_SIZE <= 0 || fd > OPEN_MAX)
		return (NULL);
	if (previous_ls[fd] && !previous_ls[fd][0])
		ft_clear(previous_ls[fd], NULL);
	tmp = get_next_from_previous(fd, previous_ls[fd]);
	if (!tmp || (!tmp[0] && !previous_ls[fd]))
	{
		ft_clear(tmp, previous_ls[fd]);
		previous_ls[fd] = NULL;
		return (NULL);
	}
	target_l = just_first_line(tmp);
	ft_clear(previous_ls[fd], NULL);
	previous_ls[fd] = get_next_lines(tmp, target_l);
	ft_clear(tmp, NULL);
	return (target_l);
}
